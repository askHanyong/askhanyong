...
3. Prints ONLY "PASS" as its final line if the check succeeds, or "FAIL: <short reason>" as its final line if it does not.
Only import sympy and Python's math/cmath standard library -- no other packages, no file or network access, no input().
Return ONLY the raw Python code, no markdown fences, no prose before or after.
...
export async function verifyWithSympy(q: GeneratedQuestionJson): Promise<SympyVerification> {
  ...
  const cleaned = script.replace(/^```(python)?\n?/i, '').replace(/```$/, '').trim();
